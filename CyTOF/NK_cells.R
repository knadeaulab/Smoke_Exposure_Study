...
md = read.csv("FileInfoNKcells.csv")
...
########## NK cells #############
#################################
md$Fpath = paste("Data/HandGated/NK_Cells/", md$Filename, sep="")
...
PIDs = do.call(c, IDs)
##save(ExpressionMatrix, lineage_markers, functional_markers, md, PIDs, finalSamples, cd3, file = "Expmatrix_NKcells.Rdata")
load("Expmatrix_NKcells.Rdata")
...
gg = ggplot(prop, aes(x = Group, y = Prop, color = Group)) + 
  ...
  labs(subtitle = paste("NK cells | P-value", Pvalue) )

pdf("NKcells_Boxplot_prop_total_CD3neg.pdf", width = 1.8, height = 4)
print(gg)
dev.off()
...
lm = c("CCR6","CD56" ,"CD16","CD27", "CD11c", "CD62L", "HLA.DR", "PD.1") 
...
clusOrder = c(5, 1,2,3,4,6,7,8) ##manually elected order to show CCR7-CD45RA+ cells above naive cells 
mm = match(clusOrder, pdf7$cluster)
pdf7$cluster = factor(pdf7$cluster , levels = clusOrder)


pdf("NKcell_FlowSOM_clusters_Heatmap.pdf", width = 8, height = 5)
...
##save(list.out, data, lineage_markers, subsample.expr, idx, PIDs, file="NKcell_UMAP.Rdata")
...
pdf("NKcell_UMAP_clusters.pdf", width = 5.5, height = 5 )
print(g)
...
pdf("NKcells_Cluster_cell_Prop_BoxPlot.pdf", width = 10, height = 11)
wrap_plots(plotlist = bplots, ncol = 6, nrow = 3)
dev.off()

write.csv(out_prop, file = "NKcells_Cluster_cell_Prop_BoxPlot.pvalue.csv", quote = F)
...
MSI_analysis <- function(data, clus, functional_markers, md2, toxicMetals){ 
  ...
  for (mark in fm){
    ...
    if ((sum(tmpdf2$Marker) > 0.1) & (tg == 2)) {
      ...
      clrs = c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0')
      
      gg = ggplot(tmpdf2, aes(x = Group, y = log(Marker))) + 
        geom_boxplot(position = position_dodge(width = 0.85), outlier.shape = NA) +
        geom_point(aes(color = Batch),position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.7), size = 2, alpha = 0.8) +
        #scale_fill_manual(values = c('#4363d8','#f58231',"black")) +
...
pdf("NKcell_cluster_MSI_BoxPlot.pdf", width =2, height = 4.2)
for (p in bxplots){
  print(p)
}
dev.off()

write.csv(out_MSI, file = "NKcell_cluster_MSI_BoxPlot.pvalue.csv", quote = F)
...
bxplotsmetals  = list()
maincluster = unique(c(data$cluster))
...
pdf("NKcell_cluster_Metals_MSI_BoxPlot.pdf", width =5, height = 7.2)
...
for (clus in mainClusters){ 
  ...
  out_fdr = out[[2]]
  filename = paste("NK_", clus, "_prop.pdf", sep="")
...
write.csv(ggdfg, file = "NKcells_MSI_P_value_per_marker_per_clus.csv", quote = F, row.names = F)
...
write.csv(seurat_output, file = "NKcells_Seuret_P_value_per_marker_per_clus.csv", quote = F, row.names = F)
...
#################################
load("NKcells_subsets.Rdata")
# NK cd56bright #
# foldername = "Data/HandGated/NK_Cells_CD56_CD16/NK_CD56bright/"
...
# NKcd56_Exp_matrix0 = getExpressionMatrix(mdp=md00)
# NKcd56_Exp_matrix0 = dplyr::filter(NKcd56_Exp_matrix0, SampleID %in% md2$SampleID)

# # CD56dimCD16pos NK #
# foldername = "Data/HandGated/NK_Cells_CD56_CD16/NK_CD56dimCD16pos/"
# fcsfiles = list.files(path = foldername, pattern = "fcs", ignore.case = T)
# matched_indices <- sapply(md$RegEx, function(pattern) {
#   grep(pattern, fcsfiles, ignore.case = TRUE)
# })
...
# NKcd16_Exp_matrix0 = getExpressionMatrix(mdp=md01)
# NKcd16_Exp_matrix0 = dplyr::filter(NKcd16_Exp_matrix0, SampleID %in% md2$SampleID)
...
#save(NKcd56_Exp_matrix0, NKcd16_Exp_matrix0, md00, md01, file = "NKcells_subsets.Rdata")

#################################################
##### Marker+ cells in gated sub-population #####
#################################################
fm = intersect(functional_markers, colnames(NKcd16_Exp_matrix0))
NKcd16_Exp_matrix0$cluster = "a"
out0 = extract_key_cells(NKcd16_Exp_matrix0, fm , clus='a', md2)
out_surt0 = extract_key_cells_seuret(NKcd16_Exp_matrix0, fm, clus='a', md2)
outMSI0 = MSI_analysis(NKcd16_Exp_matrix0, clus='a', fm, md2)

NKcd56_Exp_matrix0$cluster = "a"
out1 = extract_key_cells(NKcd56_Exp_matrix0, fm , clus="a", md2)
out_surt1 = extract_key_cells_seuret(NKcd56_Exp_matrix0, fm, clus='a', md2)
outMSI1 = MSI_analysis(NKcd56_Exp_matrix0, clus='a', fm, md2)



write.csv(out0[[2]], file = "NKcells_NKcd16pos_P_value_per_marker.csv", quote = F, row.names = F)
write.csv(out1[[2]], file = "NKcells_NKcd56bright_P_value_per_marker.csv", quote = F, row.names = F)

write.csv(out_surt0, file = "NKcells_NKcd16pos_seuret_P_value_per_marker.csv", quote = F, row.names = F)
write.csv(out_surt1, file = "NKcells_NKcd56bright_P_value_per_marker.csv", quote = F, row.names = F)

write.csv(outMSI0[[1]], file = "NKcells_NKcd16pos_MSI_P_value_per_marker.csv", quote = F, row.names = F)
write.csv(outMSI1[[1]], file = "NKcells_NKcd56bright_MSI_P_value_per_marker.csv", quote = F, row.names = F)



pdf("NKcd16pos.pdf", width = 1.6, height = 3.5)
for (plt in out0[[1]]){print(plt)}
dev.off()
pdf("NKcd56bright.pdf", width = 1.6, height = 3.5)
for (plt in out1[[1]]){print(plt)}
dev.off()

pdf("NKcd16pos_MSI.pdf", width = 1.6, height = 3.5)
for (plt in outMSI0[[2]]){print(plt)}
dev.off()
pdf("NKcd56bright_MSI.pdf", width = 1.6, height = 3.5)
