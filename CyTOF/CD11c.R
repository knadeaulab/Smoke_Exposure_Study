...
md = read.csv("FileInfoCD11cpos.csv")
...
########## CD11cPos cells #######
#################################
md$Fpath = paste("Data/HandGated/CD11cPos/", md$Filename, sep="")
...
PIDs = do.call(c, IDs)
save(ExpressionMatrix, lineage_markers, functional_markers, md, PIDs, finalSamples, cd3, file = "Expmatrix_CD11poscells.Rdata")
#load("Expmatrix_CD11poscells.Rdata")
...
pdf("CD11cPoscells_Boxplot_prop_total_CD3neg.pdf", width = 1.8, height = 4)
print(gg)
dev.off()
...
flowSOM.res <- FlowSOM(fcs.input, colsToUse = lineage_markers, scale = FALSE ,nClus=10) ## 15 clusters
...
pdf("CD11cPoscell_FlowSOM_clusters_Heatmap.pdf", width = 8, height = 5)
...
save(list.out, data, lineage_markers, subsample.expr, idx, PIDs, file="CD11cPoscell_UMAP.Rdata")
...
pdf("CD11cPoscell_UMAP_clusters.pdf", width = 5.5, height = 5 )
print(g)
...
pdf("CD11cPoscells_Cluster_cell_Prop_BoxPlot.pdf", width = 10, height = 11)
wrap_plots(plotlist = bplots, ncol = 6, nrow = 3)
dev.off()

write.csv(out_prop, file = "CD11cPoscells_Cluster_cell_Prop_BoxPlot.pvalue.csv", quote = F)
############################################
...
## using only markers expressed by CD11cPos-CELLS 
functional_markers = c("CD27","CD28","CD154", "CD294","CD185", "CD62L", "CD183", "CD194", "CD196", "CD197", "CD195","CD279",
                       "CXCR5","CXCR3","CCR4","CCR6","CCR7","CCR5","PD.1", "CRTH2","79Br","81Br","209Bi","204Pb", "206Pb", "208Pb","196Hg", "198Hg", "202Hg","182W","184W", 
...
pdf("CD11cPoscell_cluster_MSI_BoxPlot.pdf", width =2, height = 4.2)
for (p in bxplots){
  print(p)
}
dev.off()
...
write.csv(out_MSI, file = "CD11cPoscell_cluster_MSI_BoxPlot.pvalue.csv", quote = F)
...
for (clus in mainClusters){ 
  ...
  out = extract_key_cells(data, keymarkers=fm , clus, md2)
  out_fdr = out[[2]]
...
write.csv(ggdfg, file = "CD11cPoscells_MSI_P_value_per_marker_per_clus.csv", quote = F, row.names = F)
...
write.csv(seurat_output, file = "CD11cPoscells_Seuret_P_value_per_marker_per_clus.csv", quote = F, row.names = F)
